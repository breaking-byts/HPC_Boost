...
def spread_score(time_series: np.ndarray) -> float:
    ...
    if iqr == 0:
        return 0.0  # Constant events provide no anomaly detection value
...
def correlation_score(event_idx: int, all_events_matrix: np.ndarray) -> float:
    ...
    # Constant events have 0 variance, hence 0 utility
    if np.std(target) == 0:
        return 0.0
...
These baselines represent the prior state-of-the-art that HPC-Boost must beat. All baselines must expose a `select_events` or `transform` method so they can be swapped easily in the experiment pipeline.

### Step 5.1 — 2SMaRT Baseline (Global Selection)

Create `src/baselines/two_smart.py`:
The 2SMaRT methodology (Sayadi, DATE 2019) selects a single *global* set of HPC events that maximize detection across all binaries. It uses Pearson correlation to find events with the highest variance and correlation to anomalous behavior.
...
class TwoSmartBaseline:
    """
    2SMaRT methodology: Selects the Top-K global events based on 
    overall variance and correlation to the malware/benign label across the entire dataset.
    """
    def __init__(self, k: int = 4):
        self.k = k
        self.selected_events = []
        
    def fit(self, X_all: pd.DataFrame, y_labels: np.ndarray, event_columns: List[str]):
        """
        X_all: DataFrame containing all time-series data for all samples.
        y_labels: Binary labels (0 = benign, 1 = malicious) for each row.
        """
        correlations = {}
        for col in event_columns:
            # Drop constant columns to avoid warnings
            if X_all[col].std() == 0:
                correlations[col] = 0.0
                continue
            
            # Absolute Pearson correlation with the target label
            corr = X_all[col].corr(pd.Series(y_labels))
            correlations[col] = abs(corr) if pd.notna(corr) else 0.0
            
        # Select top K events globally
...
Create `src/baselines/pca_selection.py`:
Instead of selecting physical registers, this method uses PCA to project the 54-dimensional event space down to K synthetic dimensions (Kadiyala, TECS 2020).
...
class PCABaseline:
    """
    PCA-based dimensionality reduction. Projects the full HPC event space
    into K synthetic principal components.
    """
    def __init__(self, k: int = 4):
        self.k = k
...
Create `src/baselines/global_fixed.py`:
This baseline represents the "naive" approach used by many early papers: hardcoding the 4 most commonly available events.
...
class GlobalFixedBaseline:
    """
    Selects the 4 most universally common architectural events.
    Because RaDaR uses specific PMU names, we map these conceptually.
    """
    def __init__(self):
        # 1. Total Instructions: 'Instruct' or 'Ins_Retd'
        # 2. Total Cycles: 'Core_cyc'
        # 3. Cache Misses: 'L1D_Miss'
        # 4. Branch Misses: 'BrMispred'
...
Create `src/baselines/random_selection.py`:
Used as a control to prove that our mathematical ranking is better than random chance.
...
class RandomBaseline:
    """Selects K events completely at random."""
    def __init__(self, k: int = 4, seed: int = 42):
        self.k = k
        self.seed = seed
        
    def select_events(self, event_columns: List[str]) -> List[str]:
        random.seed(self.seed)
        return random.sample(event_columns, self.k)
```
