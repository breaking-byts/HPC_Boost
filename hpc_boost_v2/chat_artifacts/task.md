...
- [x] Implement Score 1: **Spread Score** (IQR-based, median across runs)
- [x] Implement Score 2: **Trend Score** (Mann-Kendall S statistic consistency)
- [x] Implement Score 3: **Correlation Score** (Spearman inter-event independence)
- [x] Implement Score 4: **Stationarity Score** (ADF/PP/KPSS with short/long distinction)
- [x] Implement **combined ranking** (normalized score aggregation)
- [x] Unit tests for each score function
- [ ] Apply ranking to Zhou dataset → generate per-binary event rankings
- [x] Apply ranking to RaDaR dataset (50-sample subset test)
- [x] Add constant-event diagnostic and penalty (prevent `inf` scores for events with 0 variance like `ILenStal`)
- [ ] Apply ranking to full 3,471-sample RaDaR dataset (Deferred to overnight batch run)
- [x] Validate: do the top-ranked events differ across binaries? (Confirmed 100% uniqueness on subset)
